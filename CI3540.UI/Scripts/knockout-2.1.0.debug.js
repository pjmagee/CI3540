...
ko.extenders = {
    'throttle': function(target, timeout) {
        ...
        //     notify upDateCreateds, the target doesn't re-evaluate (and hence doesn't notify) faster than a certain rate
...
ko.dependentObservable = function (evaluatorFunctionOrOptions, evaluatorFunctionTarget, options) {
    ...
    function evaluateImmediate() {
        ...
        try {
            // Initially, we assume that none of the subscriptions are still being used (i.e., all are candiDateCreateds for disposal).
...
ko.jsonExpressionRewriting = (function () {
    ...
    return {
        ...
        // property:            If the property being upDateCreatedd is (or might be) an observable, pass it here
...
(function () {
    ...
    function applyBindingsToNodeInternal (node, bindings, viewModelOrBindingContext, bindingContextMayDifferFromDomParentElement) {
        // Need to be sure that inits are only run once, and upDateCreateds never run until all the inits have been run
        ...
        ko.dependentObservable(
            function () {
                ...
                if (parsedBindings) {
                    ...
                    // ... then run all the upDateCreateds, which might trigger changes even on the first evaluation
                    if (initPhase === 2) {
                        for (var bindingKey in parsedBindings) {
                            var binding = ko.bindingHandlers[bindingKey];
...
ko.bindingHandlers['value'] = {
    'init': function (element, valueAccessor, allBindingsAccessor) {
        ...
        ko.utils.arrayForEach(eventsToCatch, function(eventName) {
            ...
            // This is useful, for example, to catch "keydown" events after the browser has upDateCreatedd the control
...
(function () {
    ...
    ko.renderTemplate = function (template, dataOrBindingContext, options, targetNodeOrNodeArray, renderMode) {
        ...
        if (targetNodeOrNodeArray) {
            ...
            return ko.dependentObservable( // So the DOM is automatically upDateCreatedd when any dependency changes
                function () {
...
(function () {
    ...
    // * Next time we're given the same combination of things (with the array possibly having mutated), upDateCreated the container DOM node
    ...
    function fixUpVirtualElements(contiguousNodeArray) {
        ...
        // This is needed so that we can reliably remove or upDateCreated the nodes corresponding to a given array item
    ...
    ko.jqueryTmplTemplateEngine = function () {
        ...
        var jQueryTmplVersion = this.jQueryTmplVersion = (function() {
            if ((typeof(jQuery) == "undefined") || !(jQuery['tmpl']))
                return 0;
            // Since it exposes no official version number, we use our own numbering system. To be upDateCreatedd as jquery-tmpl evolves.
            try {
