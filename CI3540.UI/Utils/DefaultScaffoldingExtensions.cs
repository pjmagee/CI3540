...
namespace CI3540.UI.Utils
{
    public static class DefaultScaffoldingExtensions
    {
        ...
        public static string IdentifierPropertyName(this Type type)
        {
            ...
            {
                return type.GetProperties().First(info => info.PropertyType.AttributeExists<System.ComponentModel.DataAnnotations.KeyAttribute>()).Name;
            }
            
            if (type.GetProperties().Any(p => p.Name.Equals("id", StringComparison.CurrentCultureIgnoreCase)))
            {
                return type.GetProperties().First(p => p.Name.Equals("id", StringComparison.CurrentCultureIgnoreCase)).Name;
            }
            
            if (type.GetProperties().Any(p => p.Name.StartsWith(type.Name, StringComparison.CurrentCultureIgnoreCase) && p.Name.EndsWith("id", StringComparison.CurrentCultureIgnoreCase)))
            {
                return type.GetProperties().First(p => p.Name.StartsWith(type.Name, StringComparison.CurrentCultureIgnoreCase) && p.Name.EndsWith("id", StringComparison.CurrentCultureIgnoreCase)).Name;
            }
